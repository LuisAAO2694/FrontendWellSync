...
//#region src/pages/registros/[id]/editar.astro
...
var $$Editar = createComponent(async ($$result, $$props, $$slots) => {
	...
	if (Astro.request.method === "POST") {
		...
		let registro = null;
		try {
			registro = await apiFetch(`/api/registros-diarios/${id}`, { token });
		} catch (e) {
			if (e instanceof ApiError && e.statusCode === 401) {
				clearSessionCookie(Astro.cookies);
				return Astro.redirect("/login");
		...
		const completadosExistentes = new Set(registro.habitosCompletados.filter((hc) => hc.completado).map((hc) => hc.habito));
		valores = {
